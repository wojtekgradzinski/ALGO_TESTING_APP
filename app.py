...
if uploaded_file is not None:
    ...
    symbol = st.sidebar.selectbox(label="FOREX & CRYPTO & CFDs", options=["AUDJPY", "AUDUSD", "EURCHF", "EURGBP", "EURJPY", "EURUSD", "GBPUSD", "NZDUSD", 
                "USDCAD","USDCHF","USDJPY","SILVER","GOLD","NAS","DAX","CAC","FTSE", "DOW","BTC","LTC","ETH", "DSH", "TRX", "EOS"])    
    ...
    tc = float(st.sidebar.text_input("Transaction Cost", 0.000001))
    st.sidebar.header("MODEL BACKTESTER") 
    backtest_init = st.sidebar.checkbox('Run Backtester')
    backtest_options = ["BACK TO MEAN","SMA CROSSOVER", "RSI MODEL"]#"LOGISTIC REGRESSION", "DNN_PHANTOM"] 
    ...
    if backtest_options == "SMA CROSSOVER":
        ...
        SMA_SLOW = int(st.sidebar.text_input("Moving Average (SLOW)",200))
    ...
    #     train_ratio =   float(st.sidebar.text_input('Train Data',0.9))
    #     lags =   int(st.sidebar.text_input('Features',5)) 
    ...
    if backtest_options == "BACK TO MEAN":
        SMA_s = int(st.sidebar.text_input("SMA Strat",150))
        SMA_e = int(st.sidebar.text_input("SMA End",180))
        dev_s = int(st.sidebar.text_input('Standard Deviation Start',2))
        dev_e = int(st.sidebar.text_input('Standard Deviation End',5))
    ...
    if backtest_options == "SMA CROSSOVER":
        ...
        SMA_slow_e = int(st.sidebar.text_input('SMA Slow  End',150))
        if optimization == True:
            SMA_crossover.optimize_parameters((SMA_fast_s,SMA_fast_e),(SMA_slow_s,SMA_slow_e))
            st.write(SMA_crossover.results_overview)
            st.pyplot(SMA_crossover.plot_results()) 
            
    # if backtest_options == "LOGISTIC REGRESSION":
    #     lags =   int(st.sidebar.text_input('Opt Features',10))
    #     if optimization == True:
    ...
    if backtest_options == "RSI MODEL":
        ...
        RSI_upper_e = int(st.sidebar.text_input('RSI Upper End',80))
        RSI_lower_s = int(st.sidebar.text_input('RSI Lower Start',20))
        RSI_lower_e = int(st.sidebar.text_input('RSI Lower End',30))
        Time_s = int(st.sidebar.text_input('RSI Time Period Start',10))
