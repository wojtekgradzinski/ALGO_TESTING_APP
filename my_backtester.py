...
#%%
    ...
    def test_strategy(self):
        ...
        #determine number of trades in each bar
        data['trades'] = data.position.diff().fillna(0).abs()
              
         # subtract transaction/trading costs from pre-cost return 
        data.strategy = data.strategy - data.trades * self.tc
        
        #computing cum returns
        data['creturns'] = data['returns'].cumsum().apply(np.exp)
        data['cstrategy'] = data['strategy'].cumsum().apply(np.exp)
        self.results = data
        
        perf = data['cstrategy'].iloc[-1] #absolute performance of the strategy
        outperf = perf - data['creturns'].iloc[-1] #out/undeperformance of strategy
    ...
    def optimize_parameters(self, SMA_range, dev_range):
        ...
        opt = combinations[np.argmax(results)]   #best parameters
        
        #run startegy again with best parameters
        self.set_parameters(opt[0], opt[1])
        self.test_strategy()
        
        #create df with many results
